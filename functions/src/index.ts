...
const transporter = nodemailer.createTransport({
  ...
  secure: true,
  auth: {
    user: functions.config().spacemail.user,
    pass: functions.config().spacemail.pass,
  },
});

export const sendDemoBooking = functions.https.onCall(async (data, context) => {
  const {
    name,
    email,
    company,
  ...
  await transporter.sendMail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px; color: #1e293b;">
          ...
          <table style="width: 100%; border-collapse: collapse;">
            ...
            <tr>
              ...
              <td style="padding: 8px 0;">
                <a href="mailto:${email}" style="color: #1abc9c;">${email}</a>
              </td>
            </tr>
            <tr>
        ...
        <a href="${meetLink}" style="color: #1abc9c;">🔗 Meet Link</a>
      </div>
    `,
  });
